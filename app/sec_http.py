...
site:
  ...
  1. A descriptive User-Agent identifying who is making the request. Requests
     without one are refused with a bare 403.
  2. Rate limiting. SEC's published ceiling is 10 req/sec; we stay well under.

Kept separate from edgar.py so both tickers.py and edgar.py can depend on it
without a cycle: transport has no knowledge of domain objects.
"""
